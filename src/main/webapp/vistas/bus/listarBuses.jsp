...
<body>
    ...
    <a href="<%= request.getContextPath() %>/bus?accion=nuevo">Registrar nuevo bus</a>
    ...
    <table border="1">
        ...
        <tr>
            ...
            <td>
                <a href="<%= request.getContextPath() %>/bus?accion=editar&id=<%= bus.getIdBus() %>">Editar</a>
                <%
                    //Solo se muestra la opcion de desactivar si el bus
                    //sigue activo; uno ya desactivado no necesita este enlace
                    if (bus.isActivo()) {
                %>
                    | <a href="#" onclick="confirmarDesactivar(<%= bus.getIdBus() %>)">Desactivar</a>
                <% } %>
            </td>
        </tr>
        <%
